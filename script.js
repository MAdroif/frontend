...
class AppState {
  ...
  saveEditedSlide(slideIndex, dataUrl) {
    this.editedSlides.set(slideIndex, dataUrl);
  }
  
  getEditedSlide(slideIndex) {
    return this.editedSlides.get(slideIndex);
  }
    ...
    infoDiv.innerHTML = `
      <div class="flex items-center justify-between">
        ...
        <div class="flex gap-1">
          <button onclick="openEditor('${slide.download_url || slide.url}', ${index}, ${JSON.stringify(slides).replace(/"/g, '&quot;')})" 
...
let currentEditSlides = null;
let isDrawingMode = false;

async function openEditor(imageUrl, index, slides) {
  ...
  try {
    ...
    fabricCanvas.setWidth(img.width * scale + 100);
    fabricCanvas.setHeight(img.height * scale + 100);
...
async function saveEditedSlide() {
  ...
  try {
    showToast('Saving edited slide...', 'info');
    ...
    const link = document.createElement('a');
    link.href = dataURL;
    link.download = `edited-slide-${currentEditIndex + 1}.jpg`;
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
    
    // Update slides array with edited image
    if (currentEditSlides && currentEditIndex !== null) {
      currentEditSlides[currentEditIndex].url = dataURL;
      currentEditSlides[currentEditIndex].download_url = dataURL;
    }
...
document.addEventListener('keydown', function(e) {
  ...
  if (e.key === 'Escape') {
    ...
    e.preventDefault();
    // Note: For full undo/redo, implement fabric history
  }
});
...
document.addEventListener('keydown', function(e) {
  const editorModal = document.getElementById('editor-modal');
  if (!editorModal.classList.contains('hidden') && e.key === 'Escape') {
    closeEditor();
  }
});
// ==================== INITIALIZE APP ====================
